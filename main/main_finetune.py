...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            # Accuracyグラフ
            plt.subplot(1, 2, 1)
            plt.plot(epoch_list, accuracy_list, linestyle='-', marker='o')
            plt.title('Accuracy vs. Epochs')
            plt.xlabel('Epochs')
            plt.ylabel('Accuracy')
            plt.grid(True)
            plt.xticks(rotation=45)  # x軸ラベルの回転
            plt.locator_params(axis='x', nbins=10)  # x軸ラベルの数を調整
            plt.tick_params(axis='both', which='both', direction='in', labelrotation=45)
            plt.minorticks_on()
            ...
            # Lossグラフ
            plt.subplot(1, 2, 2)
            plt.plot(epoch_list, loss_list, linestyle='-', marker='o')
            ...
            # 保存
            plt.savefig(os.path.join(args.output_dir, "figures", f"training_stats_epoch_{epoch + 1}.png"))
            plt.close()
            # ↑
